...
func _ready():
	if ResourceLoader.exists("res://fire_axe_sprite.tscn"):
		var sprite_scene = load("res://fire_axe_sprite.tscn")
...
func _draw():
	if show_range and GameSettings.show_hitboxes:
		...
		draw_rect(Rect2(rect_pos, rect_size), Color(1, 0.3, 0, 0.4))
	...
	if results.size() > 0:
		for result in results:
			...
			if hit_body != owner_player and hit_body.has_method("take_damage"):
				...
				print("🪓 Fire Axe hit: ", hit_body.name, " for ", damage, " damage!")
	else:
		print("🪓 Fire Axe swing missed!")
