...
func _ready():
	if ResourceLoader.exists("res://shovel_script.tscn"):
		var sprite_scene = load("res://shovel_script.tscn")
		sprite = sprite_scene.instantiate()
		add_child(sprite)
	else:
		print("Shovel sprite not found!")

func _draw():
	if show_range and GameSettings.show_hitboxes:
		var rect_pos = Vector2(0, -width/2)
		var rect_size = Vector2(range, width)
		draw_rect(Rect2(rect_pos, rect_size), Color(1, 0.5, 0, 0.3))
		draw_rect(Rect2(rect_pos, rect_size), Color.ORANGE, false, 2)
	...
	if results.size() > 0:
		for result in results:
			...
			if hit_body != owner_player and hit_body.has_method("take_damage"):
				...
				print("Shovel hit: ", hit_body.name, " for ", damage, " damage!")
	else:
		print("Shovel swing missed!")
